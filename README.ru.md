<p align="center">
  <a href="https://github.com/nekopunk44/nekopunk44/blob/main/README.md">
    <img src="https://img.shields.io/badge/language-EN-374151?style=for-the-badge" alt="English" />
  </a>
  <a href="https://github.com/nekopunk44/nekopunk44/blob/main/README.ru.md">
    <img src="https://img.shields.io/badge/language-RU-0b1220?style=for-the-badge" alt="Russian" />
  </a>
</p>

<p align="center">
  <img src="https://media.giphy.com/media/v1.Y2lkPWVjZjA1ZTQ3aGpqbDNuZnoxMDZmeXQ0ZjJ3dTFpNHZyemF3dGhucGVmZHVpaTgyNiZlcD12MV9naWZzX3NlYXJjaCZjdD1n/d3mm8K5DCeigO3Go/giphy.gif" alt="portfolio.os profile strip" />
</p>

<p align="center">
  <a href="https://nekopunk.vladbredihin4.workers.dev">
    <img src="https://img.shields.io/badge/live-portfolio-0b1220?style=for-the-badge&logo=cloudflare&logoColor=f59e0b" alt="Live portfolio" />
  </a>
  <a href="https://t.me/van_g0ck">
    <img src="https://img.shields.io/badge/telegram-@van__g0ck-0b1220?style=for-the-badge&logo=telegram&logoColor=7ddfff" alt="Telegram" />
  </a>
  <a href="mailto:vladbredihin4@gmail.com">
    <img src="https://img.shields.io/badge/email-vladbredihin4%40gmail.com-0b1220?style=for-the-badge&logo=gmail&logoColor=e5e7eb" alt="Email" />
  </a>
</p>

```txt
~/whoami
Владислав Бредихин
Fullstack-разработчик, создающий продуктовые сайты, интерфейсы с motion-подачей
и fullstack-системы, которым нужно больше, чем типовая шлифовка.
```

## `~/about`

Мне нравятся проекты, которые ощущаются собранными, запоминающимися и коммерчески острыми.

- Продуктовый frontend с более сильной презентацией
- Fullstack-реализация от идеи до деплоя
- Интерфейсы, где motion и иерархия действительно работают
- Концепты, которым нужны и инженерия, и вкус

## `~/selected-work`

- [Live portfolio](https://nekopunk.vladbredihin4.workers.dev)  
  Главная витрина моего текущего terminal-style портфолио.

- [MomentumX](https://github.com/nekopunk44/MomentumX)  
  Agency-style платформа с фокусом на айдентику, упаковку оффера и более сильную презентацию.

- [ZenPulse-AI-Meditation-App-Prototype](https://github.com/nekopunk44/ZenPulse-AI-Meditation-App-Prototype)  
  Спокойный мобильный product concept вокруг AI-позиционирования и более чистого UX storytelling.

- [Avtotime](https://github.com/nekopunk44/Avtotime)  
  Более крупный fullstack-проект с комьюнити, marketplace и realtime-сценариями.

<details>
  <summary><code>~/stack</code></summary>

  <br />

  `TypeScript` `React` `Next.js` `Node.js` `.NET` `MongoDB` `PostgreSQL` `Docker` `Three.js`
</details>

<details>
  <summary><code>~/current-focus</code></summary>

  <br />

  - Более чистые кейсы для GitHub
  - Сильнее frontend motion без ощущения шаблона
  - Более цельная визуальная айдентика между репозиториями
  - Проекты, которые ощущаются как продукты, а не как свалки файлов
</details>

## `~/reach-out`

- Telegram: [@van_g0ck](https://t.me/van_g0ck)
- GitHub: [github.com/nekopunk44](https://github.com/nekopunk44)
- Email: `vladbredihin4@gmail.com`

> Я не хочу, чтобы проекты просто работали. Я хочу, чтобы они оставляли впечатление.
